"""Data loaders for the de novo sequencing or SSL spectrum modeling tasks."""
...
class DeNovoDataModule(pl.LightningDataModule):
    """
    Data loader to prepare MS/MS spectra for a Spec2Pep predictor or SpectrumSSL pretraining.
    """
    ...
    def __init__(
        ...
        self.preprocessing_fn = [
            ...
            preprocessing.filter_intensity(min_intensity, max_peaks), # intensity从大到小排序了
    ...
    # 🚀 修改部分：setup()
    # =====================================================
    def setup(self, stage: str = None, annotated: bool = False) -> None:
        """
        ...
        -----
        ✅ 修改内容：
        - 默认 annotated=False（用于 SpectrumSSL，自监督不需 peptide 序列）
        - 强制 SpectrumDataset，不再加载 tokenizer
        """
        if stage in (None, "fit", "validate"):
            if self.train_paths is not None:
                self.train_dataset = self._make_dataset(
                    self.train_paths,
                    annotated=False,  # ✅ 原版是 True，现在改成 False
                    mode="train",
        ...
        logger.info("✅ Using SpectrumDataset (no peptide annotations)")
    ...
    def _make_dataset(
        ...
        # ✅ 修改：无论 annotated=True/False，强制使用 SpectrumDataset
