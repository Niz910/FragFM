...
class Spec2Pep(pl.LightningModule):
    ...
    *args,
    ...
    # 对齐：预测前 L-1 token，用来预测下一个
    pred = pred[:, :-1, :].contiguous().view(-1, self.vocab_size)
    targ = truth[:, 1:].contiguous().view(-1)

    if mode == "train":
        loss = self.celoss(pred, targ)
    else:
        loss = self.val_celoss(pred, targ)
    ...
    self.log(
        ...
        on_epoch=True,
        sync_dist=True,
        batch_size=batch["mz_array"].shape[0],  # 用真实 batch size
    )
    return loss
